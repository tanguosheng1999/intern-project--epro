...
if ($signed === $data['x_signature']) {
    if ($paidstatus == "Success"){ //payment success
        ...
        $sqlcart = "SELECT * FROM `tbl_admincarts` INNER JOIN tbl_customers ON tbl_admincarts.customer_id = tbl_customers.customer_id WHERE  tbl_admincarts.admin_email = '$email'";
        ...
        $sqldeletecart = "DELETE FROM `tbl_admincarts` WHERE admin_email = '$email'";
        try {
        $conn->exec($sqlinsertpayment);
        $stmt = $conn->prepare($sqldeletecart);
        $stmt->execute();
            echo "<script>alert('Payment successful')</script>";
            echo "<script>window.location.replace('https://eproesystem.com/esystem/epro/php/indexadmin.php')</script>";
